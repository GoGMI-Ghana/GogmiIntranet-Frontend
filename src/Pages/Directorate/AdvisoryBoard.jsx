...
export default function AdvisoryBoard() {
  ...
  const handleArchive = async (id) => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/advisory-board/${id}/archive`, {
        method: 'PUT',
        headers: getAuthHeaders()
      });
      const data = await response.json();
  ...
  const handleUnarchive = async (id) => {
    try {
      const response = await fetch(`${API_URL}/api/advisory-board/${id}/unarchive`, {
        method: 'PUT',
        headers: getAuthHeaders()
      });
      const data = await response.json();
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-lg shadow-md p-12 text-center">
            ...
            <p className="text-gray-500">
              {activeTab === 'current'
                ? 'Members will appear here as soon as they submit the Advisory Board form.'
