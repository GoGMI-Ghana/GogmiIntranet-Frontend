...
export default function MarineCasualtyCourse() {
  ...
  const fetchRegistrations = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${API_URL}/api/marine-casualty-course/registrations`, {
  ...
  const handleExportCSV = async () => {
    try {
      ...
      const url = `${API_URL}/api/marine-casualty-course/export/csv?${params.toString()}`;
      const response = await fetch(url, { headers: getAuthHeaders() });
      const blob = await response.blob();
      const downloadUrl = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = downloadUrl;
      a.download = `marine_casualty_${selectedYear}_${selectedSection}.csv`;
      a.click();
    } catch (error) {
  ...
  const handleExportExcel = async () => {
    try {
      ...
      const url = `${API_URL}/api/marine-casualty-course/export/excel?${params.toString()}`;
      const response = await fetch(url, { headers: getAuthHeaders() });
      const blob = await response.blob();
      const downloadUrl = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = downloadUrl;
      a.download = `marine_casualty_${selectedYear}_${selectedSection}.xlsx`;
      a.click();
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Marine Casualty Investigation Course</h1>
          <p className="text-gray-600">Track registrations and revenue (Member: $400, Non-Member: $500)</p>
        </div>
