...
import { Users, Download, Search, Globe, DollarSign, UserCheck, Calendar, Mail, Phone, Building2, Briefcase } from 'lucide-react';
...
const MaritimeGovernanceCourse = () => {
  ...
  const fetchRegistrations = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/maritime-course/registrations`, {
        headers: {
  ...
  const handleExportCSV = () => {
    const token = localStorage.getItem('token');
    window.open(`${import.meta.env.VITE_API_URL}/api/maritime-course/export/csv?token=${token}`, '_blank');
  };

  const handleExportExcel = () => {
    const token = localStorage.getItem('token');
    window.open(`${import.meta.env.VITE_API_URL}/api/maritime-course/export/excel?token=${token}`, '_blank');
  };
    ...
    <div className="min-h-screen bg-gray-50 p-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            <nav className="text-sm text-gray-500 mb-2">
              ...
              <span>General</span>
              <span className="mx-2">/</span>
              <span className="text-gray-900 font-medium">Maritime Governance Course</span>
            </nav>
            <h1 className="text-3xl font-bold text-gray-900">Maritime Governance Course Registrations</h1>
            <p className="text-gray-600 mt-1">May 5-15, 2026 | A Focus on Africa</p>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-6 mb-8">
        ...
        <div className="bg-white p-6 rounded-lg border border-gray-200">
          ...
          <p className="text-xs text-gray-500 mt-1">$350 each</p>
        </div>
        ...
        <div className="bg-white p-6 rounded-lg border border-gray-200">
          ...
          <p className="text-xs text-gray-500 mt-1">$450 each</p>
        </div>
